#!/usr/bin/env bash
#
# justrade-dev.sh - start / stop / restart the local dev stack:
#   an N-node Aeron Raft cluster (+ archive), a CQRS read replica that follows
#   node-0's archive, and the HTTP/WS gateway.
#
# Usage:
#   ./scripts/justrade-dev.sh {start|stop|restart|status|seed|bench}
#
# Env overrides (all optional):
#   JUSTRADE_NODES           number of cluster nodes        (3)
#   JUSTRADE_INGRESS_BASE    node-0 ingress port            (20100); node n uses +n*100
#   JUSTRADE_HOST            host advertised in members     (localhost)
#   JUSTRADE_QUERY_PORT      read query channel port        (44000)
#   JUSTRADE_HTTP_PORT       gateway HTTP port              (8080)
#   JUSTRADE_RUN_DIR         pid/log/state base dir         (/tmp/justrade)
#   JUSTRADE_CLEAN_START     wipe cluster state on start    (true; restart uses false by default)
#   JUSTRADE_SYMBOLS         gateway.symbols                (BTC/USDT, ETH/USDT sample)
#   JUSTRADE_CURRENCIES      gateway.currencies             (BTC=10, USDT=20 sample)
#   JUSTRADE_ADMIN_UIDS      gateway.admin.uids             (1,2,811)
#   JUSTRADE_BENCH_OPS       gateway bench commands         (10000)
#   JUSTRADE_BENCH_USERS     gateway bench users            (100)
#   JUSTRADE_BENCH_SYMBOLS   gateway bench symbol count     (1)
#   JUSTRADE_ADMIN_UID       gateway bench admin uid        (811)
#   JUSTRADE_ADMIN_API_KEY   gateway.admin.apiKey           (unset = disabled)
#
# First run builds the application distributions (installDist) automatically.
# State and logs live under $JUSTRADE_RUN_DIR; PID files under $JUSTRADE_RUN_DIR/pids.
...
JUSTRADE_NODES="${JUSTRADE_NODES:-3}"
BASE_PORT="${JUSTRADE_INGRESS_BASE:-20100}"
HOST="${JUSTRADE_HOST:-localhost}"
QUERY_PORT="${JUSTRADE_QUERY_PORT:-44000}"
HTTP_PORT="${JUSTRADE_HTTP_PORT:-8080}"
RUN_DIR="${JUSTRADE_RUN_DIR:-/tmp/justrade}"
CLEAN_START="${JUSTRADE_CLEAN_START:-true}"
SYMBOLS="${JUSTRADE_SYMBOLS:-1|BTC/USDT|10|20|1|1|0|0,2|ETH/USDT|10|20|1|1|0|0}"
CURRENCIES="${JUSTRADE_CURRENCIES:-10|BTC|1,20|USDT|1}"
ADMIN_UIDS="${JUSTRADE_ADMIN_UIDS:-1,2,811}"
BENCH_OPS="${JUSTRADE_BENCH_OPS:-10000}"
BENCH_USERS="${JUSTRADE_BENCH_USERS:-100}"
BENCH_SYMBOLS="${JUSTRADE_BENCH_SYMBOLS:-1}"
ADMIN_UID="${JUSTRADE_ADMIN_UID:-811}"
ADMIN_API_KEY="${JUSTRADE_ADMIN_API_KEY:-}"
...
LAUNCHER_MAIN=io.justrade.launcher.ClusterLauncher
READ_MAIN=io.justrade.read.ReadServiceLauncher
GATEWAY_MAIN=io.justrade.gateway.GatewayLauncher
LAUNCHER_LIB="$ROOT/launcher/build/install/launcher/lib"
READ_LIB="$ROOT/read/build/install/read/lib"
GATEWAY_LIB="$ROOT/gateway/build/install/gateway/lib"
BENCH_LIB="$ROOT/bench/build/install/bench/lib"
BENCH_MAIN=io.justrade.bench.GatewayBenchRunner
...
build_members() {
  local m="" p
  for ((i = 0; i < JUSTRADE_NODES; i++)); do
    p=$((BASE_PORT + i * 100))
    m+="${i},${HOST}:${p},${HOST}:$((p + 1)),${HOST}:$((p + 2)),${HOST}:$((p + 3)),${HOST}:$((p + 4))"
    if (( i + 1 < JUSTRADE_NODES )); then m+="|"; fi
  done
  printf '%s' "$m"
}
...
ingress_list() { # Aeron cluster-client ingress: id=host:port,...
  local id p list=""
  for ((id = 0; id < JUSTRADE_NODES; id++)); do
    p=$((BASE_PORT + id * 100))
    [ -n "$list" ] && list+=","
    list+="${id}=${HOST}:${p}"
  done
...
ensure_dists() {
  if [ ! -f "$LAUNCHER_LIB/launcher.jar" ] \
      || [ ! -f "$READ_LIB/read.jar" ] \
      || [ ! -f "$GATEWAY_LIB/gateway.jar" ]; then
    log "building application distributions (installDist) ..."
    "$ROOT/gradlew" --quiet :launcher:installDist :read:installDist :gateway:installDist
  fi
}

ensure_bench_dist() {
  if [ ! -f "$BENCH_LIB/bench.jar" ]; then
    log "building bench distribution (installDist) ..."
    "$ROOT/gradlew" --quiet :bench:installDist
  fi
}
...
justrade.clusterMembers=$(build_members)
justrade.host=${HOST}
justrade.baseDir=${CLUSTER_BASE}/node-$i
EOF
  done
...
start_cluster() {
  ...
  for ((i = 0; i < JUSTRADE_NODES; i++)); do
    ...
    java "${ADD_OPENS[@]}" \
      -Djustrade.nodeId="$i" -Djustrade.cleanStart="$CLEAN_START" \
...
case "$cmd" in
  ...
  restart)
    stop_all
    # Restart preserves cluster state unless JUSTRADE_CLEAN_START is set explicitly.
    CLEAN_START="${JUSTRADE_CLEAN_START:-false}"
    sleep 2
    ensure_dists
    write_confs
    start_cluster
