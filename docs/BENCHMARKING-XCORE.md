...
The `xcore-bench` module measures justrade against the upstream
[exchange-core](https://github.com/exchange-core/exchange-core) 0.5.3 - the
engine whose matching semantics justrade ports. It exists to answer one
...
| Mode     | justrade side                                  | exchange-core side                          |
|----------|-------------------------------------------------|---------------------------------------------|
...
- **Risk coverage differs by layer.** At the `book` layer neither side does
  ...
  placement, settle at fill price, release on cancel). The justrade engine
  path additionally runs per-client dedup; exchange-core has no equivalent at
  the matching layer.
- **Allocation asymmetry.** The justrade engine path reuses SBE flyweights and
...
- **Consensus is not free.** The `e2e` justrade number pays for Raft log
  commit and archive recording on every command; exchange-core has no
  replication. The gap between `engine` and `e2e` on the justrade side is the
...
Cross-validation is built into the replay: justrade's book is the reference,
...
# JMH comparison of the order books (same shapes as core's OrderBookBenchmark
# plus a fresh-book replay chunk shaped after exchange-core's ITOrderBookBase)
./gradlew :xcore-bench:jmh
./gradlew :xcore-bench:jmh -PquickBench
./gradlew :xcore-bench:jmh -Pjmh.profilers=gc
```
...
justrade book's advantage and understate the pipeline's steady-state
...
treat the justrade `engine` mode as the counterpart of their engine-only
figures.
