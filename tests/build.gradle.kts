...
dependencies {
    ...
    testImplementation(project(":xcore-bench"))
...
tasks.withType<Test>().configureEach {
    ...
    // SystemLoadIntegrationTest scale knob: -Djustrade.systemload.ops / -Djustrade.systemload.users
    // lets the docker-equivalent run be reproduced at full scale in one JVM.
    systemProperty("justrade.systemload.ops", System.getProperty("justrade.systemload.ops", "20000"))
    systemProperty("justrade.systemload.users", System.getProperty("justrade.systemload.users", "20"))
    // ChaosSoakTest scale knob: -Djustrade.soak.warmupRounds / -Djustrade.soak.steadyRounds
    // (one round is one step of an 8-step workload pattern; 8 rounds = 13 commands).
    systemProperty("justrade.soak.warmupRounds", System.getProperty("justrade.soak.warmupRounds", "15000"))
    systemProperty("justrade.soak.steadyRounds", System.getProperty("justrade.soak.steadyRounds", "120000"))
}
...
val coveredProjects = listOf(":core", ":write-client", ":launcher", ":read", ":read-client", ":gateway")
