...
echo "excoredum load: ${OPS} ops / ${USERS} users / ${SYMBOLS} symbols against ${ENDPOINTS} (egress ${EGRESS})"
exec java \
    $JAVA_OPTS \
    ...
    --endpoints="${ENDPOINTS}" --egress="${EGRESS}" --ops="${OPS}" --users="${USERS}" --symbols="${SYMBOLS}"
