...
public final class ClusterConfig {
    ...
    public static final String TERM_LENGTH_PROPERTY = "justrade.aeron.termLength";
     ...
     *
     * <p>Recognised keys: {@code justrade.clusterMembers} (required, member format),
     * {@code justrade.baseDir} (directory root, default {@code build/justrade-node-<id>}),
     * and {@code justrade.host} (this node's host, default {@code localhost}).
     */
    ...
    public static ClusterConfig fromProperties(final Properties props, final int nodeId) {
        final String members = props.getProperty("justrade.clusterMembers");
        if (members == null || members.isBlank()) {
            throw new IllegalArgumentException("justrade.clusterMembers is required");
        }
        final Path baseDir = Paths.get(props.getProperty("justrade.baseDir", "build/justrade-node-" + nodeId));
        final String host = props.getProperty("justrade.host", "localhost");
     ...
     * string, this node's {@code host}, and a directory {@code baseDir}.
     */
